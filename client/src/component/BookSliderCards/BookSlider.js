...
const BookCard = ({ title, books }) => {
  ...
  return (
    <div className="w-full h-full px-1 sm:px-4 md:px-5 lg:px-28 my-10    ">
      <h2 className="text-4xl  md:text-5xl w-fit mx-auto md:text-center font-bold text-black my-5">
        <div>{title}</div>
      ...
      >
        {books.map((book) => (
          <SwiperSlide key={book._id} className="BookCard  ">
            <Link
              to={`/book/${book._id}`}
              className=" w-[200px] md:w-[90%]  h-[70%] md:h-fit lg:h-[77%]  overflow-hidden hover:shadow-lg text-left"
            >
              ...
              <div className=" py-1 px-2 flex flex-col h-[] text-left md:text-left ">
                ...
                <p className="text-xs text-gray-600 font-nato-sans mb-2">{book.author}</p>
